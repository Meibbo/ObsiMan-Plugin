...
- Metric evidence exists in `metrics/pkm-ai.jsonl`; latest health event
  is `health_passed` with `0 warnings`.
- serviceViews implementation plan exists:
  [[docs/work/hardening/plans/2026-05-04-serviceviews-implementation/index|serviceViews plan]].
...
- Bases interop research continued with API details and a compatibility matrix: [[docs/work/hardening/research/2026-05-05-bases-interop-research/index|bases interop research]].
...
- User selected [[docs/work/hardening/specs/2026-05-06-user-facing-recovery-wave-a/index|User-facing recovery wave A]] as the first deferred-work wave.
- A0 CMenu queue repair completed: [[docs/work/hardening/plans/2026-05-06-cmenu-queue-repair/index|CMenu queue repair implementation]].
  Tags/files CMenu actions now stage queue work; `file_delete` trashes only on queue execute.
- A1 Prop/Value Rename Handoff completed; follow-up repaired prop queue
  ingestion by emitting `NATIVE_RENAME_PROP` from FnR prop rename changes.
...
- Current verification: `serviceFnR`, `serviceQueue`, `explorerTags`,
  `explorerFiles`, `pageFiltersRenameHandoff`; focused unit/component tests,
