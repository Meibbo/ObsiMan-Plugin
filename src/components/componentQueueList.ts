...
export class QueueListComponent {
	...
	private renderOpRow(
		...
		const isExpanded =
			this.callbacks.expandedPath === vfs.originalPath && this.callbacks.expandedOpId === op.id;
	...
	private isSameLogicalOp(left: StagedOp, right: StagedOp): boolean {
		return (
			left.kind === right.kind && left.action === right.action && left.details === right.details
		);
	}
}
